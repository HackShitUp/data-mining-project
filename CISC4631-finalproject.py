...
dt_predict = dt_classifier.fit(train_features_balanced, train_labels_balanced).predict(test_features)
...
rf_predict = rf_classifier.fit(train_features_balanced, train_labels_balanced).predict(test_features)


# In[17]:


print(confusion_matrix(test_labels, rf_predict))
...
knn_predict = knn_classifier.fit(train_features_balanced, train_labels_balanced).predict(test_features)
...
gnb_predict = gnb_classifier.fit(train_features_balanced, train_labels_balanced).predict(test_features)
...
for i in range(len(dt_predict)):
    final.append(mode([dt_predict[i], rf_predict[i], knn_predict[i], gnb_predict[i]]))
wrongs = 0
